...
### Иерархия принцесс (отдельная ветка)

| Class | Type | Attributes | Methods |
|-------|------|------------|---------|
| Фиона | Concrete | + имя = "Принцесса Фиона", + являетсяОгром: bool (зависит от времени), + человеческаяФорма: String, + формаОгра: String | + трансформироваться(), + сопротивляться(), + ждатьСпасения(), + {static} проверитьВремяСуток() |
...
| Class | Type | Attributes | Methods |
|-------|------|------------|---------|
| СостояниеЧеловек | Abstract | - | + действовать(), + трансформироватьсяВОгра() |
| СостояниеОгр | Abstract | - | + действовать(), + трансформироватьсяВЧеловека() |
